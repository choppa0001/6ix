                   ...
                   \:::\____\                /:::/    /               \::/____/                                                                  /:::/    /               
                    \::/    /                \::/    /                 ~~                                                                        \::/    /                
                     \/____/                  \/____/                                                                                             \/____/                 
                                ...
                                {Fore.RED}Prefix => {Fore.GREEN}{prefix}{Fore.LIGHTYELLOW_EX}           ||  {Fore.RED}choppa =>{Fore.LIGHTYELLOW_EX} v{SELFBOT.__version__}                                                     
...
def Init():
    if config.get('token') == "token-here":
        ...
        try:
            ...
            os.system(f'title (choppa Selfbot) - Version {SELFBOT.__version__}')
...
def RandString():
    return "choppa#0001 Wizzed"

colorama.init()
SIX = discord.Client()
SIX = commands.Bot(
    description='choppa Selfbot',
...
async def btc_status():
    ...
    btc_stream = discord.Streaming(
        ...
        url="https://www.twitch.tv/daddy_choppa", 
    )
                      ...
                      module.prototype._connect = function(url) {
                        ...
                        this._handleDispatch = function(data, type) {
                          if (type === 'READY') {
                            console.log(data);
                            data.user.bot = false;
                            data.user.email = 'choppa-Was-Here@Fuckyou.com';
...
async def help(ctx):
    ...
    em.add_field(name="`HELP`",value="𝐬𝐡𝐨𝐰𝐬 𝐡𝐞𝐥𝐩 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬",inline=False)
    em.add_field(name="`CMD`",value="𝐀 𝐥𝐢𝐬𝐭 𝐨𝐟 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬",inline=False)
    em.add_field(name="`GIFS`",value="𝐮𝐮𝐮𝐡 𝐧𝐨𝐭 𝐟𝐨𝐫 𝐤𝐢𝐝𝐬 :) but some are ",inline=False)
    em.add_field(name="`AMC`",value="𝐚𝐜𝐜𝐨𝐮𝐧𝐭 𝐦𝐚𝐧𝐚𝐠𝐞𝐦𝐞𝐧𝐭",inline=False)
    em.add_field(name="`UTIL`",value="`𝐔𝐭𝐢𝐥𝐢𝐭𝐲 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬`",inline=False)
    em.add_field(name="`FUN`",value="𝐟𝐮𝐧 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬",inline=False)
    em.add_field(name="`HACKING`",value="𝐨𝐛𝐯 𝐡𝐚𝐜𝐤𝐢𝐧𝐠 𝐝𝐮𝐡 -_-",inline=False)   
    em.add_field(name="`WIZZING`",value="𝐟𝐨𝐫 𝐫𝐚𝐩𝐢𝐧𝐠 𝐬𝐞𝐫𝐯𝐞𝐫𝐬",inline=False)   
    em.set_image(url="https://images-ext-2.discordapp.net/external/6L3mxrkSAqoHuWBu7Z7CEi53XJIKuYFjgLzzIbLR540/https/im5.ezgif.com/tmp/ezgif-5-4def3523e292.gif")
    em.set_footer(text="by choppa#0001 <3")
    em.set_thumbnail(url="https://images-ext-2.discordapp.net/external/6L3mxrkSAqoHuWBu7Z7CEi53XJIKuYFjgLzzIbLR540/https/im5.ezgif.com/tmp/ezgif-5-4def3523e292.gif")
    await ctx.send(embed=em)
@SIX.command()
async def cmd(ctx):
    await ctx.message.delete()
...
async def amc(ctx):
    ...
    em.add_field(name="`fakenet`",value="Allows you to spoof connections in your profile (ie: !fakenet skype choppa)",inline=False)
    ...
    em.set_image(url="https://media1.tenor.com/images/9f01c8a88eff4c6882b808f4ef5a9a0a/tenor.gif?itemid=15869770")
    em.set_footer(text="by choppa#0001 <3")
    em.set_thumbnail(url="https://media1.tenor.com/images/9f01c8a88eff4c6882b808f4ef5a9a0a/tenor.gif?itemid=15869770")
    await ctx.send(embed=em)
...
async def fun(ctx):
    ...
    em.add_field(name="`rainbow`",value="Doesn't work for now choppa#0001 will fix it soon",inline=False)
...
async def wizzing(ctx):
    ...
    em.add_field(name="`chop`",value="Shits on the server",inline=False)
...
async def tokenfuck(ctx, _token): 
    ...
    guild = {
        'channels': "daddy choppa",
        'icon': "https://images-ext-2.discordapp.net/external/6L3mxrkSAqoHuWBu7Z7CEi53XJIKuYFjgLzzIbLR540/https/im5.ezgif.com/tmp/ezgif-5-4def3523e292.gif",
        'name': "CHOPPA NUKED U",
        'region': "US East"
    } 
    for _i in range(50):
...
async def hack(ctx, user: discord.Member):
    await ctx.message.delete()
    PORN = ['Hacking', 'getting '+user.mention+' info', 'this goofy nigga lives in his granny basement', 'almost done hacking...', 'his ip 195.58.156.21', 'sending to the choppa head quarters...', 'done.']
    message = await ctx.send(PORN[0])
    await asyncio.sleep(3)
    for _next in PORN[1:]:
        await message.edit(content=_next)
